...
export default function Projects() {
  // Track open architecture subpanels by indexing them
  const [openArchIndex, setOpenArchIndex] = useState(null);

  const toggleArchitecture = (index) => {
    setOpenArchIndex(openArchIndex === index ? null : index);
  };
      ...
      <div className="space-y-12 max-w-5xl mx-auto">
        {projectsData.map((project, index) => (
          <motion.div 
            ...
            className="group relative bg-slate-900/80 backdrop-blur-md rounded-2xl p-6 md:p-8 shadow-[0_4px_30px_rgba(0,0,0,0.4)] flex flex-col hover:border-slate-400/40 transition-colors duration-500 border border-slate-800/80 overflow-hidden"
          >
            ...
            <div className="flex flex-col md:flex-row gap-8 items-start w-full relative z-10">
              ...
              <div className="w-full md:w-52 flex flex-col justify-between h-full md:self-stretch border-t md:border-t-0 md:border-l border-slate-800 pt-6 md:pt-0 md:pl-8 space-y-6">
                ...
                <div className="flex flex-col gap-2.5">
                  ...
                  <Button 
                    variant="secondary" 
                    onClick={() => toggleArchitecture(index)}
                  >
                    {openArchIndex === index ? "Hide Architecture" : "System Architecture"}
                  </Button>
                </div>
              </div>
            </div>

            {/* Dropdown Project Architecture Details Panel */}
            <AnimatePresence>
              {openArchIndex === index && (
                <motion.div 
                  ...
                  className="w-full border-t border-slate-800 mt-6 pt-6 relative z-10"
                >
                  <div className="bg-slate-950/40 border border-slate-800/60 rounded-xl p-5">
                    ...
                    <ul className="space-y-2.5">
                      {project.architecture.details.map((detail, detailIdx) => (
                        <li key={detailIdx} className="text-xs text-slate-300 flex items-start gap-2 leading-relaxed">
